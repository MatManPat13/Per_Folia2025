...
document.addEventListener("DOMContentLoaded", function () {
  ...
  ventanas.forEach(ventana => {
    const textoVentana = ventana.querySelector(".contenido-ventana p");
    if (textoVentana) prepararAnimacionVentana(textoVentana);
  });

  const botonesAbrir = document.querySelectorAll('[id^="abrirVentana"]');

  botonesAbrir.forEach((boton, index) => {
    boton.addEventListener("click", () => {
      let ventanaId = "ventanaEmergente" + (index === 0 ? "" : index);
      const ventana = document.getElementById(ventanaId);

      if (ventana) {
        const textoVentana = ventana.querySelector(".contenido-ventana p");
        if (textoVentana) {
          textoVentana.querySelectorAll("span").forEach(span => {
            span.style.animation = "none";
            span.style.opacity = "0";
            span.style.filter = "blur(4px)";
          });

          setTimeout(() => animarElementoVentana(textoVentana), 50);
        }
      }
    });
  });
